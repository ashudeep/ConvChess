...
class Sunfish_Mod2(Player):
    def __init__(self, maxn=1e4):
        self._pos = sunfish_mod2.Position(sunfish.initial, 0, (True,True), (True,True), 0, 0)
        self._maxn = maxn
        #self.first_move = True
    ...
    def move(self, gn_current):
        if gn_current.board().turn == 1 :
            ...
            # Apply last_move
            crdn = str(gn_current.move)
            move = (sunfish_mod2.parse(crdn[0:2]), sunfish.parse(crdn[2:4]))
            self._pos = self._pos.move(move)
            #self.first_move = False
            #t0 = time.time()
            ...
            crdn = sunfish_mod2.render(119-move[0]) + sunfish_mod2.render(119 - move[1])
        else:
            if gn_current.move is not None:
                # Apply last_move
                crdn = str(gn_current.move)
                move = (119 - sunfish_mod2.parse(crdn[0:2]), 119 - sunfish_mod2.parse(crdn[2:4]))
                self._pos = self._pos.move(move)
            #t0 = time.time()
            move, score = sunfish_mod2.search(self._pos, maxn=self._maxn)
            #print time.time() - t0, move, score
            ...
            crdn = sunfish_mod2.render(move[0]) + sunfish_mod2.render(move[1])
        move = create_move(gn_current.board(), crdn)  
        gn_new = chess.pgn.GameNode()
        gn_new.parent = gn_current
        gn_new.move = move
